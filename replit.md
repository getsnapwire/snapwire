...
## Recent Changes
- **2026-02-25**: Streamlined Setup Wizard — (1) Merged Welcome + Environment Check into single screen (auto-runs health check inline). (2) Removed Registration step entirely (moved to Settings for later). (3) Setup flow reduced from 5 clicks to 3: Welcome → Create Account → Accept ToS → Dashboard. (4) `pyproject.toml`: added `[build-system]` section for reliable Docker builds. (5) Removed `/auth/setup-register` route.
- **2026-02-25**: One-Click Deployment Matrix — (1) Landing page Self-Hosted card: added Docker `git clone && docker compose up` one-liner with copy button below the "Fork on Replit" CTA. (2) README: deploy badges (Replit, Railway, Docker) added below tagline; "One-Line Install" section with `docker compose up` command. (3) `railway.toml` added for Railway one-click deploys (Dockerfile builder, `/health` check, restart policy). (4) Branding sweep: fixed 7 remaining "Safety Fuse" references across `pricing.html`, `audit.html`, `email_service.py` (welcome, first-block, magic link emails), `nist_mapping.py` API disclaimer, `README.md`, `pyproject.toml`, `sdk/README.md`, `rules/__init__.py`.
- **2026-02-25**: Pre-Launch Branding & Copy Updates — (1) Landing page badge, title, and footer updated from "The Real-Time Circuit Breaker" / "The Safety Fuse" to **"The Firewall for AI Agents"**. Hero headline kept as-is ("Stop Your AI Agents Before They Break Something"). (2) Dashboard NIST gauge updated: "NIST AI RMF" → "NISTIR 8596", "Readiness" → "Cyber AI Profile", description references CSF 2.0 Govern · Protect · Detect functions. LinkedIn share text updated. (3) Weekly digest email: subject → "Your Weekly Circuit Breaker Report: {blocked} Triggers, ${savings} Saved", header → "Weekly Circuit Breaker Report", footer → "The Firewall for AI Agents".
...
- **2026-02-26**: BYOK Enforcement — (1) `src/llm_provider.py` `chat()`: when `tenant_id` is provided but no tenant LLM key configured, raises `RuntimeError` instead of falling back to platform Anthropic key. Platform key only used for `tenant_id=None` calls (public audit). (2) Public audit rate limit tightened from 10/hr to 3/hr per IP. (3) All LLM callers updated to propagate `tenant_id`: `auditor.py`, `deception.py` (`analyze_deception`), `nlp_rule_builder.py` (`parse_natural_language_rule`, `detect_rule_conflicts`, `test_rule_against_action`), `tool_catalog.py` (`grade_tool`, `check_tool_catalog`, `regrade_tool`). (4) Intercept endpoint catches `RuntimeError` from BYOK-missing users gracefully — allows action through with advisory note instead of 500 error. Other endpoints (`/api/rules/parse`, `/api/rules/conflicts`, `/api/catalog/regrade`) return 400 with clear BYOK message.
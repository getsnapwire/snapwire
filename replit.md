...
- **2026-02-27**: Fork-Ready Launch Assets — (1) `src/safeguard_openclaw.py`: deterministic BASE_URL redirect attack detector (CVE-2026-25253), 6 threat patterns (base_url override, credential exfiltration, domain spoofing, env var injection, suspicious TLD, config override), hooked into `/api/intercept` pipeline. (2) `tests/cve_2026_25253_repro.py`: 12 reproduction tests proving exploit blocked. (3) `X-Snapwire-Signature`: HMAC-SHA256 header in Sentinel proxy for cryptographic proof of governance, with `X-Snapwire-Timestamp` for replay protection. (4) `.github/workflows/nist-audit.yml`: GitHub Action runs tests + NIST export on every PR, posts compliance grade as comment. (5) `CONTRIBUTING.md` rewritten with Sentinel protocol contribution path, AI-PR policy, Contributor's First Day guide. (6) `examples/basic_agent.py`: minimal URL-swap demo agent. (7) `.github/ISSUE_TEMPLATE/`: new-protocol, bug-report, feature-request templates. (8) `scripts/quickstart.sh`: 30-second smoke test script. Total: 94 tests passing (80 core + 14 CVE repro).

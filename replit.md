...
-   **Engine Room**: A super-admin dashboard with system health monitoring, batch ingestor UI, chaos lab, global burn meter, stealth control, telemetry, HITL evidence, detector lab, latency monitor, shadow agents, Agent Locks, and Fork Tracker.
-   **Three-Tier Role Model**: Platform Admin, Workspace Owner, and Viewer roles.
-   **Fork Tracking**: `/badge/snapwire.svg` route logs Referer + UA + IP to `badge_pings` table. Engine Room `er-forks` sub-tab shows live GitHub fork data (via public API) + badge deployment ping log. Optional `GITHUB_TOKEN` env var increases GitHub API rate limit.
-   **Agent-Collision Module**: `AgentLock` model with mutex-for-meaning semantics. API: `POST /api/agent-lock/acquire` (returns 409 if locked), `POST /api/agent-lock/release`, `GET /api/agent-lock/status`, `POST /api/agent-lock/<id>/force-release`. Engine Room "Agent Locks" sub-tab.
-   **JIT Snap-Tokens**: `allowed_tools` (JSON list) and `jit_intent` (text) columns on `proxy_tokens`. Intercept endpoint blocks any tool not in the allowed list (HTTP 403, `error_code: jit_tool_not_allowed`). JIT modal in the Vault panel per entry.

**Key Features & Implementations:**
-   **Runtime Security**: Fuse Breaker (loop detection), Schema Validation Guard, Snap-Tokens with JIT context-matching (secure proxy/revocation), Live Burn Meter (cost tracking), Risk Confidence Index, Tool Safe Catalog (AI-powered safety grading), Blast Radius Governor (dual-limit systems), Identity Vault (credential storage), Agent-Collision Detection (mutex-for-meaning DB locks), Deception & Goal Drift Detector, Honeypot Tripwires, OpenClaw (redirect attack detector), Taint Tracking (cross-call data-flow governance), Session Pulse (continuous token re-validation), Deterministic Hold Window (for high-risk calls), Strict Reasoning Toggle.

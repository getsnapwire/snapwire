...
-   **Slack Integration**: slack-bolt (Socket Mode alerts)
...
- **2026-02-27**: Fork-Ready Launch Assets — (1) `src/safeguard_openclaw.py`: deterministic BASE_URL redirect attack detector (CVE-2026-25253), 7 threat patterns (base_url override, credential exfiltration, domain spoofing, env var injection, suspicious TLD, config override, websocket hijacking), hooked into `/api/intercept` pipeline. (2) `tests/cve_2026_25253_repro.py`: 17 reproduction tests proving exploit blocked. (3) `X-Snapwire-Signature`: HMAC-SHA256 header in Sentinel proxy for cryptographic proof of governance, with `X-Snapwire-Timestamp` for replay protection. (4) `.github/workflows/nist-audit.yml`: GitHub Action runs tests + NIST export on every PR, posts compliance grade as comment. (5) `CONTRIBUTING.md` rewritten with Sentinel protocol contribution path, AI-PR policy, Contributor's First Day guide, NIST Grade badge snippet. (6) `examples/basic_agent.py`: minimal URL-swap demo agent. (7) `.github/ISSUE_TEMPLATE/`: new-protocol, bug-report, feature-request templates. (8) `scripts/quickstart.sh`: 30-second smoke test script. Total: 101 tests passing (84 core/sentinel + 17 CVE repro).
- **2026-02-27**: Liability-Shielding Features — (1) `/safety` route: public Safety Disclosure page (Colorado SB24-205 compliance) with dynamic NIST grade, coverage score, active safeguards count, 7 disclosure sections (Intended Use, Foreseeable Misuse, Human Accountability, Algorithmic Discrimination, Compliance Standards, NIST Grade, Active Safeguards). (2) `X-Snapwire-Authorized-By` header in Sentinel proxy: human accountability attribution per Singapore MGF 2026, configurable via `SENTINEL_AUTHORIZED_BY` env var, defaults to `origin_id`. (3) WebSocket origin validation in OpenClaw: blocks `ws://`/`wss://` URLs to non-allowlisted hosts and WebSocket upgrade headers targeting unauthorized domains. (4) `/badge/nist-grade` endpoint: embeddable SVG badge showing NIST grade (A/B/C/D) with color coding, cacheable, for README embedding.
- **2026-02-27**: Vanguard Onboarding Readiness — (1) `scripts/quickstart.sh` now outputs `[NIST AUDIT: PASSED (Grade A)]` as final line with full audit summary (Enforce Mode, HMAC Signing, CVE block status, Identity Headers, NIST Grade). (2) Sentinel proxy injects `X-Snapwire-Origin-ID` and `X-Snapwire-Parent-ID` headers in ALL modes (observe/audit/enforce) when tool calls are detected — satisfies NIST IR 8596 Continuous Passive Provenance. (3) `/badge/nist-grade` SVG badge changed from "Snapwire | NIST Grade X" to "NIST-Aligned | March 2026" with grade-based color coding. Total: 85 core/sentinel tests passing + 17 CVE repro = 102 total.
- **2026-02-27**: Sentinel Forensic Metadata in Audit Log + README NIST RFI — (1) When tool calls arrive via Sentinel proxy, `authorized_by`, `hmac_active`, `trace_id`, and `protocol` are extracted from the payload metadata and stored in the `chain_of_thought` JSON blob under a `"sentinel"` key. All 13 `log_action` call sites in the intercept pipeline pass `sentinel_metadata`. (2) `README.md`: added "Joint Federal Filing: NIST March 9th RFI" section with 4-step participation guide, core technical argument, and badge embed snippet. (3) New test `test_sentinel_metadata_stored_in_audit_log` verifies end-to-end forensic chain. Total: 85 core/sentinel tests passing + 17 CVE repro = 102 total.
- **2026-02-27**: Vanguard Launch Readiness (Session 2) — (1) `src/safety_pdf.py`: Safety Disclosure PDF export using fpdf2 with Snapwire branding, NIST grade/score, coverage breakdown, 7 disclosure sections, SHA-256 audit fingerprint, disclaimer; `GET /safety/pdf` endpoint + download button on `/safety` page. (2) `src/thinking_sentinel.py`: `check_latency_anomaly()` detects pipeline latency anomalies via per-agent rolling average (3x threshold) or absolute 30s threshold; records `thinking-sentinel-latency` audit events; 3 new tests. (3) Deterministic Hold Window: `hold_window_seconds` on `TenantSettings`, `hold_expires_at` on `PendingAction`; high-risk allowed calls (risk >= 70) held for configurable seconds with HTTP 202; auto-release via `auto_deny_expired`; `DELETE /api/actions/{id}` cancel endpoint; `GET/PATCH /api/settings/hold-window` API; Dashboard Security tab slider (0-60s). (4) `src/slack_notifier.py`: Slack Socket Mode alerts via slack-bolt; Block Kit messages with Approve/Kill buttons for held actions; graceful skip when tokens not configured; button handlers resolve actions via `resolve_action()`. (5) `slack-bolt` package installed. Total: 88 core tests + 17 CVE repro = 105 passing.
- **2026-02-27**: Sentinel Hardening — (1) `MAX_BODY_SIZE` (10 MB) limit in `sentinel/proxy.py`: rejects oversized requests with 413 before reading full body; handles both Content-Length and chunked transfers. (2) `sentinel/Dockerfile`: runs as non-root `sentinel` user via `groupadd`/`useradd` + `USER sentinel`. (3) 2 new tests for body size constant validation. Total: 76 tests passing.
